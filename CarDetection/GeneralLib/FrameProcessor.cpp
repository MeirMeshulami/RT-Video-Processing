...
FrameProcessor::FrameProcessor() : service(mFrameProcessQueue), isServerRunning(true) {
...
void FrameProcessor::ProcessFrames() {
    ...
    WaitForFrameProcessingThreadsToFinish();
    ...
    LOG_DEBUG("FrameReceiverThread detached.");
}
...
void FrameProcessor::ProcessFrame(YoloDetection& yoloDetection, int threadIndex) {
    ...
    {
        if (mFrameProcessQueue.TryPop(frame)) {
...
void FrameProcessor::DetectFrame(FrameWithDetails& frame, YoloDetection& yoloDetection) {
...
FrameReceiver& FrameProcessor::GetService() {
    return service;
}